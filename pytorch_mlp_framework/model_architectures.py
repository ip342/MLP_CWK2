...
class BatchNormProcessingBlock(nn.Module):
    ...
    def build_module(self):
        ...
        self.bn_0 =  nn.BatchNorm2d(num_features=self.num_filters)
        ...
        self.bn_1 =  nn.BatchNorm2d(num_features=self.num_filters)
    ...
    def forward(self, x):
        ...
        out = self.layer_dict['conv_1'].forward(out)
        out =  self.bn_1.forward(out)
        out = F.leaky_relu(out)

        return out


class BatchNormResConProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(BatchNormResConProcessingBlock, self).__init__()
